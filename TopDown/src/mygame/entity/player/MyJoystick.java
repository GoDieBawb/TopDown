 ...
 * @author t0neg0d
 */
public abstract class MyJoystick extends Element implements Control {
	...
	public MyJoystick(ElementManager screen, Vector2f position, int size) {
		...
		thumb = new ButtonAdapter(screen, UIDUtil.getUID(),
                            new Vector2f(getWidth()/2-(tempV2.x/2), getHeight()/2-(tempV2.y/2)),
                                tempV2,new Vector4f(0,0,0,0),screen.getStyle("Common").getString("blankImg")) {
                                    
			@Override
			public void controlMoveHook() {
                            
                            if(getPosition().distance(origin) > maxDistance*2f) {;
                                setPosition(centerVec);
                                return;
                            }
                            
                            if (getPosition().distance(centerVec) > maxDistance) {
